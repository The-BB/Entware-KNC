...
ifeq ($(CONFIG_USE_APK),)
	...
	(cd $$(IDIR_$(1))/CONTROL; \
		( \
			echo "$$$$CONTROL"; \
			printf "Description:\n"; echo "$$$$$(call shvar,Package/$(1)/description)" | sed -e 's,^[[:space:]]*, ,g'; \
		) > control; \
		chmod 644 control; \
		( \
			echo "#!/bin/sh"; \
	...
	# Note: If the script exits with a failure (e.g., if the user already exists),
	# the package will not be installed and apk will exit with failure, hence the
	# exit 0 at the end.
	...
	( \
		echo "#!/bin/sh"; \
		[ ! -f $$(ADIR_$(1))/postrm ] || sed '/^\s*#!/d' "$$(ADIR_$(1))/postrm"; \
		echo -e "\nexit 0"; \
